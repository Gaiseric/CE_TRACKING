...
void pantallita(struct pt *pt){
  PT_BEGIN(pt);
  limpiar(); //LIMPIAR DISPLAY'S
  digito(1); //HABILITAR DIGITO 1
  numero(num[0]);//OBTENER DIGITO 1
  delayMicroseconds(1000);
  limpiar(); //LIMPIAR DISPLAY'S
  digito(2); //HABILITAR DIGITO 2
  numero(num[1]);//OBTENER DIGITO 2
  delayMicroseconds(1000);
  limpiar(); //LIMPIAR DISPLAY'S
  digito(3); //HABILITAR DIGITO 3
  numero(num[2]);//OBTENER DIGITO 3
  delayMicroseconds(1000);
  limpiar(); //LIMPIAR DISPLAY'S
  digito(4); //HABILITAR DIGITO 4
  numero(num[3]);//OBTENER DIGITO 4
  delayMicroseconds(1000);
  PT_END(pt);
}
...
void sonido(struct pt *pt){
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    t=millis();
    PT_WAIT_WHILE(pt,(millis()-t)<noteDuration);
    //delay(noteDuration);
